...
export default function SearchReservationPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen w-screen bg-slate-900 text-white gap-10 px-16">
      <h2 className="text-5xl font-bold">{t.buscarReserva.titulo}</h2>
      ...
      <div className="w-full max-w-2xl flex flex-col gap-6">
        <input
          ...
          className="w-full px-8 py-6 text-3xl bg-slate-800 border-2 border-slate-600 focus:border-blue-500 rounded-2xl outline-none text-white placeholder-slate-500"
          autoFocus
        />
        ...
        <button
          ...
          className="w-full py-6 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-2xl font-semibold rounded-2xl transition-colors active:scale-95"
        >
      ...
      <button
        onClick={() => navigate(-1)}
        className="text-slate-500 text-xl hover:text-slate-300"
      >
        {t.geral.btnVoltar}
      </button>
    </div>
